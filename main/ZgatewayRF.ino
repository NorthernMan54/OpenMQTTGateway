...
void setupRF() {
  //RF init parameters
  Log.trace(F("RF RF_EMITTER_GPIO: %d " CR), RF_EMITTER_GPIO);
  Log.trace(F("RF RF_RECEIVER_GPIO: %d " CR), RF_RECEIVER_GPIO);
...
#  endif
  ...
  Log.notice(F("ZgatewayRF command topic: %s%s" CR), mqtt_topic, subjectMQTTtoRF);
