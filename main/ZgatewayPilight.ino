...
#  endif
  ...
  Log.trace(F("PiLight RF_EMITTER_GPIO: %d " CR), RF_EMITTER_GPIO);
  Log.trace(F("PiLight RF_RECEIVER_GPIO: %d " CR), RF_RECEIVER_GPIO);
  Log.notice(F("ZgatewayPilight command topic: %s%s" CR), mqtt_topic, subjectMQTTtoPilight);
...
void MQTTtoPilight(char* topicOri, JsonObject& Pilightdata) {
  if (cmpToMainTopic(topicOri, subjectMQTTtoPilight)) {
    Log.trace(F("MQTTtoPilight %s" CR), topicOri);
#  ifdef ZradioCC1101
    Log.trace(F("ELECHOUSE_cc1101.SetTx: %F" CR), CC1101_FREQUENCY);
